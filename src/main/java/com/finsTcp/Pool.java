...
public abstract class Pool<T> implements Closeable {
  ...
  public T getResource() {
    try {
      ...
      if (null == nse.getCause()) { // The exception was caused by an exhausted pool
        throw new SocketExhaustedPoolException(
            "Could not get a resource since the pool is exhausted", nse);
      }
      // Otherwise, the exception was caused by the implemented activateObject() or ValidateObject()
      throw new SocketException("Could not get a resource from the pool", nse);
    } catch (Exception e) {
      throw new SocketConnectionException("Could not get a resource from the pool", e);
    }
  }
  ...
  protected void returnResourceObject(final T resource) {
    ...
    try {
      ...
      throw new SocketException("Could not return the resource to the pool", e);
    }
  }
  ...
  protected void returnBrokenResourceObject(final T resource) {
    try {
      ...
      throw new SocketException("Could not return the broken resource to the pool", e);
    }
  }
  ...
  protected void closeInternalPool() {
    try {
      ...
      throw new SocketException("Could not destroy the pool", e);
    }
  }
  ...
  public void addObjects(int count) {
    try {
      ...
      throw new SocketException("Error trying to add idle objects", e);
    }
  }
}